...
const TILE_TEXTURES: Record<string, string> = {
  [ZONES.FOREST]: "tile_forest", // Key in SPRITE_MAP
};
...
export function createTile(zoneId: ZoneId, x: number, y: number) {
  ...
  if (TILE_TEXTURES[zoneId] && SPRITE_MAP[TILE_TEXTURES[zoneId]]) {
    const texKey = TILE_TEXTURES[zoneId];
    sprite = Sprite.from(SPRITE_MAP[texKey]);
  ...
  // Border (Only if using Graphics, or add overlay?)
  // If sprite, maybe we don't need border, or add a graphics child?
  // Use container if we need multiple layers?
  // For now, let's skip border on Sprite to keep it simple, or add it if it's Graphics.
